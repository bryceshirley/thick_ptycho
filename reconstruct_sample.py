...
def main(cfg_path):
    ...
    if solver_type == "PWE":
        recon = ReconstructorPWE(simulation_space, data,
                                 phase_retrieval=cfg["recon_solver"]["phase_retrieval"],
                                 bc_type=cfg["recon_solver"]["bc_type"])
    elif solver_type == "MS":
        recon = ReconstructorMS(simulation_space, data,
                                phase_retrieval=cfg["recon_solver"]["phase_retrieval"])
    else:
        raise ValueError(f"Unknown solver type: {solver_type}")

    n_est, wave_field, residual_hist = recon.reconstruct(max_iters=cfg["recon_solver"]["max_iters"])

    # Visualizations
    simulation_space.viewer.plot_two_panels(n_est,view="phase_amp",
                                            title="Reconstructed Object",
                                            filename="reconstructed_object.png")
    simulation_space.viewer.plot_residual(residual_hist,
    ...
    np.savez_compressed(n_est_file, n_est=n_est, wave_field=wave_field, residual_history=residual_hist)
