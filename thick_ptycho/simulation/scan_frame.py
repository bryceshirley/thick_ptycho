...
class Limits:
    ...
    x: Union[Tuple[int, int], Tuple[float, float]]
    y: Optional[Union[Tuple[int, int], Tuple[float, float]]] = None
    z: Optional[Union[Tuple[int, int], Tuple[float, float]]] = None
    ...
    def __post_init__(self):
        ...
        if self.units not in ("pixels", "meters", "p", "m"):
            raise ValueError(
                f"units must be either 'pixels' or 'meters', got {self.units}"
            )
        ...
        for coords in self.as_tuple():
            ...
            if self.units in ["pixels", "p"]:
                ...
                if not all(isinstance(v, (int, np.integer)) for v in coords):
                    raise ValueError(
                        f"Limits must be integers when units='pixels', got {coords}"
                    )
                ...
                # Check numeric type
                if not all(
                    isinstance(v, (int, float, np.integer, np.floating)) for v in coords
                ):
    ...
    def __post_init__(self):
        ...
        if self.reduced_limits_continuous is not None:
            if self.reduced_limits_continuous.units != "meters":
                raise ValueError(
                    "reduced_limits_continuous must have units in 'meters'."
                )
