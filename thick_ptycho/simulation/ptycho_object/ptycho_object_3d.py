...
class PtychoObject3D(BasePtychoObject):
    ...
    def load_cameraman(self, real_perturbation=1e-4, imaginary_perturbation=1e-6):  # TODO: correct for non free space medium
        ...
        refractive_index_3d = transform.resize(refractive_index_3d, (self.nx, self.ny), preserve_range=True)

        # Expand to 3D by repeating along z
        refractive_index = np.repeat(refractive_index_3d[:, :, np.newaxis], self.nz, axis=2)
