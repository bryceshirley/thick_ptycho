...
class PtychoObject2D(BasePtychoObject):
    ...
    def load_cameraman(self, real_perturbation=1e-4, imaginary_perturbation=1e-6):  # TODO: correct for non free space medium
        ...
        refractive_index_2d = data.camera().astype(np.float32)

        # Flip the image vertically
        refractive_index_2d = np.flipud(refractive_index_2d)

        
        # Normalize to [0, 1] range
        refractive_index_2d -= refractive_index_2d.min()  # Ensure zero baseline
        refractive_index_2d /= refractive_index_2d.max()  # Normalize to unit peak

        refractive_index_2d = transform.resize(refractive_index_2d, (self.nx, self.ny), preserve_range=True)

        # Expand to 3D by repeating along z
        refractive_index = np.repeat(refractive_index_2d[:, :, np.newaxis], self.nz, axis=2)
