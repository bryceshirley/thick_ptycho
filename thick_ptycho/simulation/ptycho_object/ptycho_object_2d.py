...
class PtychoObject2D(BasePtychoObject):
    ...
    def load_and_resize_refractive_index(self, file_path: str) -> np.ndarray:
        ...
        real_resized = cv2.resize(
            np.real(n), target_shape, interpolation=cv2.INTER_LINEAR
        )
        imag_resized = cv2.resize(
            np.imag(n), target_shape, interpolation=cv2.INTER_LINEAR
        )
    ...
    def create_refractive_index_of_phantom(
        ...
        img_resized = cv2.resize(
            img, (nx_inner, nz_inner), interpolation=cv2.INTER_LINEAR
        )
        ...
        full_img[pad_top : pad_top + nx_inner, pad_left : pad_left + nz_inner] = (
            img_resized
        )
