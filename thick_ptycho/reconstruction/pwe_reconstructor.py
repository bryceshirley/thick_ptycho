...
from thick_ptycho.forward_model.pwe_solver import (
    ForwardModelPWEFull,
    ...
    **initial Gerchberg–Saxton (GS)** phase retrieval step to estimate
    ...
    def __init__(
        ...
        super().__init__(
            ...
            gs_iters=gs_iters,
            log_file_name="reconstruction_log.txt",
        )

        # Forward model selection
        SolverClass = ForwardModelPWEFull if full_system else ForwardModelPWEIterative
        self.forward_model = SolverClass(
            sample_space=sample_space, probe_angles_list=probe_angles_list or [0.0]
        )
        ...
        self.block_size = self.forward_model.linear_system.block_size
        ...
        self._log(
            f"LeastSquaresReconstructorPWE initialized "
            f"({'Full' if full_system else 'Iterative'}) solver, "
            f"data={'intensity' if data_is_intensity else 'complex'}."
        )

    # -------------------------------------------------------------------------
    # Main Nonlinear CG Reconstruction Loop
    # -------------------------------------------------------------------------
    ...
    def reconstruct(
        ...
        low_pass_filter: float = 0.0,
        sparsity_lambda: float = 0.0,
    ):
        """
        Run Nonlinear Conjugate Gradient reconstruction.

        Parameters
        ----------
        n_initial : np.ndarray, optional
            Initial refractive index estimate. If None, uses uniform medium.
        max_iters : int, default=20
            Maximum number of NLCG iterations.
        tol : float, default=1e-6
            Convergence tolerance on RMSE.
        fixed_step : float, optional
            If given, uses a fixed α step size instead of adaptive αₖ.
        low_pass_filter : float, default=0.0
            Gaussian smoothing (σ) applied along z per iteration.
        sparsity_lambda : float, default=0.0
            Soft-threshold sparsity coefficient.
        ...
        residuals : list[float]
            RMSE per iteration.
        """
        known_phase = not self.data_is_intensity
        n_est = (
            n_initial.copy()
            if n_initial is not None
            else np.ones((self.block_size, self.nz), dtype=complex)
            * self.sample_space.n_medium
        )
        ...
        for it in range(max_iters):
            ...
            # Forward propagation
            uk, grad_A = self._forward_and_grad(n_est)

            # Gradient computation
            grad_re, grad_im = self._grad_least_squares(uk, grad_A, known_phase)

            # Residual (RMSE)
            rmse = self._rmse(uk)
            residuals.append(rmse)
            self._log(f"[{it+1}/{max_iters}] RMSE = {rmse:.3e}")

            if rmse < tol:
                self._log(f"Converged in {it+1} iterations.")
                break

            # NLCG direction update
            if it == 0:
                dk_re, dk_im = -grad_re, -grad_im
                grad_re_prev, grad_im_prev = grad_re, grad_im
            else:
                beta_re = self._beta_prp(grad_re, grad_re_prev)
                beta_im = self._beta_prp(grad_im, grad_im_prev)
                dk_re = -grad_re + beta_re * dk_re
                dk_im = -grad_im + beta_im * dk_im
                grad_re_prev, grad_im_prev = grad_re, grad_im

            # Step size
            alpha_re = fixed_step or self._alpha_line_search(uk, grad_A, grad_re, dk_re)
            alpha_im = fixed_step or self._alpha_line_search(uk, grad_A, grad_im, dk_im)

            # Update refractive index
            delta_n = np.zeros_like(n_est, dtype=complex)
            delta_n[:, 1:] = (
                alpha_re * dk_re.reshape((self.nz - 1, self.block_size)).T
                + 1j * alpha_im * dk_im.reshape((self.nz - 1, self.block_size)).T
            )
            n_est += delta_n

            # Optional smoothing & sparsity
            if low_pass_filter > 0:
                n_est[:, 1:] = gaussian_filter1d(
                    n_est[:, 1:], sigma=low_pass_filter, axis=1
                )
            if sparsity_lambda > 0:
                n_est = self._soft_threshold(n_est, sparsity_lambda, alpha_re)

            self._log(f"Iteration {it+1} complete ({time.time() - t0:.2f}s)")

        return n_est, residuals

    # -------------------------------------------------------------------------
    # Core Forward + Gradient Computations
    # -------------------------------------------------------------------------

    def _forward_and_grad(self, n):
        """
        Compute forward field and system Jacobian derivative.

        Parameters
        ----------
        n : np.ndarray
            Current refractive index estimate.

        Returns
        -------
        uk : np.ndarray
            Flattened propagated field (shape: num_angles*num_probes × block_size*(nz-1)).
        grad_A : scipy.sparse matrix
            Sparse derivative of A with respect to n.
        """
        uk = self._flatten(self.forward_model.solve(n=n))
        grad_A = -self.forward_model.linear_system.setup_inhomogeneous_forward_model(
            n=n, grad=True
        )
        return uk, grad_A

    def _grad_least_squares(self, uk, grad_A, known_phase=True):
        """
        Compute gradient of the least-squares loss with respect to refractive index.

        ∇L = Re[ (∂A/∂n)ᵀ (A⁻¹)ᵀ (u_exit - u_data) ]

        Parameters
        ----------
        uk : np.ndarray
            Flattened complex field per probe-angle.
        grad_A : sparse matrix
            Jacobian of forward model w.r.t. refractive index.
        known_phase : bool
            If True, uses complex field difference.
            If False, uses amplitude constraint (phase retrieval).

        Returns
        -------
        grad_re, grad_im : np.ndarray
            Real and imaginary parts of gradient vector.
        """
        grad_re = np.zeros(self.sample_space.nx * (self.sample_space.nz - 1))
        grad_im = np.zeros_like(grad_re)

        exit_err = self._exit_error(uk, known_phase)

        for i in range(self.num_probes * self.num_angles):
            backprop = self.forward_model.solve_adjoint(exit_err[i, :])
            g = grad_A @ uk[i, :]
            grad_re -= np.real(g.conj() * backprop)
            grad_im -= np.real((1j * g).conj() * backprop)

        return grad_re, grad_im

    # -------------------------------------------------------------------------
    # Error, Step Size, Beta Computation
    # -------------------------------------------------------------------------

    def _exit_error(self, uk, known_phase=True):
        """
        Compute the difference between predicted and observed exit waves.

        Parameters
        ----------
        uk : np.ndarray
            Flattened complex propagated field.
        known_phase : bool
            Whether phase of the data is known.

        Returns
        -------
        np.ndarray
            Complex error per probe and angle.
        """
        exit_waves = uk[:, -self.block_size:]
        if known_phase:
            if exit_waves.shape != self.data.shape:
                raise ValueError("Exit wave data shape mismatch.")
            return exit_waves - self.data
        else:
            fmodel = np.fft.fft(exit_waves)
            phase = np.exp(1j * np.angle(fmodel))
            sqrtI = np.sqrt(self.data)
            emodel = np.fft.ifft(sqrtI * phase)
            return exit_waves - emodel

    def _alpha_line_search(self, u, grad_A, grad_E, d):
        """
        Compute adaptive step size αₖ using the directional derivative ratio:

            αₖ = - (∇L · d) / (dᵀ H d)

        where H is the approximate Hessian via linearized forward model.

        Parameters
        ----------
        u : np.ndarray
            Flattened propagated field.
        grad_A : sparse matrix
            Jacobian of forward model w.r.t. refractive index.
        grad_E : np.ndarray
        ...
        for i in range(self.num_probes * self.num_angles):
            pert = grad_A @ u[i, :]
            delta_u = self.forward_model.solve_perturbation(pert)
            denom += np.linalg.norm(delta_u[-self.block_size:] @ d[-self.block_size:]) ** 2
        num = np.vdot(d, grad_E)
        return -num / denom if denom > 0 else 0.0

    def _beta_prp(self, g_new, g_old):
        """
        Compute Polak–Ribière+ β coefficient.

        β = max( (g_newᵀ (g_new - g_old)) / (g_oldᵀ g_old), 0 )

        Parameters
        ----------
        g_new, g_old : np.ndarray
            Current and previous gradient vectors.

        Returns
        -------
        float
            β coefficient (non-negative).
        """
        num = np.vdot(g_new, g_new - g_old)
        den = np.vdot(g_old, g_old)
        if den == 0:
            return 0.0
        beta = np.real(num / den)
        return max(beta, 0.0)
    ...
    def _rmse(self, uk):
        ...
        """
        exit_waves = uk[:, -self.block_size:]
        if self.data_is_intensity:
            I_pred = np.abs(np.fft.fft(exit_waves)) ** 2
            return np.sqrt(np.mean((I_pred - self.data) ** 2))
        else:
            return np.sqrt(np.mean(np.abs(exit_waves - self.data) ** 2))
    ...
    def _flatten(self, u):
        """
        Convert 4D (angles, probes, nx, nz) array to 2D block form
        (angles*probes, block*(nz-1)).
        """
        num_angles, num_probes = u.shape[:2]
        u = u[:, :, :, 1:]
        u = u.transpose(0, 1, 3, 2)
        return u.reshape(num_angles * num_probes, -1)

    def _soft_threshold(self, n, lam, step):
        """
        Apply complex-valued soft thresholding to enforce sparsity.
        ...
        step : float
            Current step size (αₖ).

        Returns
        -------
        np.ndarray
            Thresholded refractive index field.
        """
        real = n[:, 1:].real - self.sample_space.n_medium
        imag = n[:, 1:].imag
        n[:, 1:] = (
            self.sample_space.n_medium
            + np.sign(real) * np.maximum(np.abs(real) - lam * step, 0.0)
            + 1j * np.sign(imag) * np.maximum(np.abs(imag) - lam * step, 0.0)
        )
        return n
...
# def create_sim_data(forward_model,log):
#         # --- Precompute the “true” forward solution (no plots here) ---
#         log("Solving the true forward problem to generate the dataset...")
#         start_time = time.time()
#         u_true = convert_to_block_form(forward_model.solve())
#         data = None
#         probes_true = forward_model.linear_system.probes
#         n_true = forward_model.sample_space.n_true
#         true_exit_waves = u_true[:, -forward_model.linear_system.block_size:]

#         # If square grid, also precompute the 90 degree rotation case once
#         u_true_rot = None
#         true_exit_waves_rot = None
#         data_rot = None
#         if forward_model.rotate90:
#             n_true_rot = np.rot90(n_true, k=1)
#             u_true_rot, _ = forward_model.compute_forward_model(n_true_rot)
#             true_exit_waves_rot = u_true_rot[:, -forward_model.linear_system.block_size:]
#             forward_model.visualize_data(rotate=True)

#         end_time = time.time()
#         log(f"True Forward Solution computed in {end_time - start_time:.2f} seconds.")

#         log(f"Angle {forward_model.probe_angles_list[0]}")
#         forward_model.visualisation.plot_auto(u_true[0], view="phase_amp", layout="single")
#         if forward_model.num_angles > 1:
#             log(f"Angle {forward_model.probe_angles_list[-1]}")
#             forward_model.visualisation.plot_auto(u_true[-1], view="phase_amp", layout="single")


#         # Select exit waves according to the orientation
#         if forward_model.rotate90 and (true_exit_waves_rot is not None):
#             exit_waves = true_exit_waves_rot
#             n_for_homog_shape = n_true_rot.shape
#             title_prefix = "(rotated)"

#             n_true = n_true_rot
#         else:
#             if forward_model.rotate90:
#                 log("Warning: rotate=True requested, but rotated forward model "
#                       "was not precomputed (requires nx == nz). Using non-rotated data.")
#             exit_waves = true_exit_waves
#             n_for_homog_shape = n_true.shape
#             title_prefix = ""


#         log("Plot True Object")
#         forward_model.visualisation.plot_single(n_true, view="phase_amp", time="final",
...
#         n_homogeneous = np.ones(n_for_homog_shape, dtype=complex) * forward_model.sample_space.n_medium
#         if title_prefix:
#             # If we're visualizing the rotated case, make sure we pass the rotated
#             # n to the forward model (keep other settings identical).
#             u_homogeneous = convert_to_block_form(forward_model.solve(n=n_homogeneous))
#         else:
#             u_homogeneous = convert_to_block_form(forward_model.solve(n=n_homogeneous))

#         exit_waves_homogeneous = u_homogeneous[:, -forward_model.block_size:]
#         diff_exit_waves = exit_waves_homogeneous - exit_waves

#         # ---------- FFT-squared intensities ----------
#         data = np.zeros((forward_model.num_probes * forward_model.num_angles, forward_model.block_size))
#         diff_data = np.zeros_like(data)

#         for i in range(forward_model.num_probes * forward_model.num_angles):
#             data[i, :] = np.square(np.abs(np.fft.fft(exit_waves[i, :])))

#             if forward_model.poisson_noise:
...
#         if rotate:
#             forward_model.data_rot = data
#         else:
#             forward_model.data = data

#         if forward_model._results_dir:
