...
#         self.num_projections = self.simulation_space.num_projections
#         self.total_scans = self.num_angles * self.num_probes * self.num_projections
#         self.total_projection_scans = self.num_angles * self.num_probes
...
#         self.nx = simulation_space.nx
#         self.nz = simulation_space.nz
#         self.projection_size = self.block_size * (self.nz - 1)

#     # -------------------------------------------------------------------------
#     # Utility methods
#     # -------------------------------------------------------------------------

#     def apply_exit_wave_constraint(self, uk, projection_idx: int=0) -> np.ndarray:
#         """
#         Compute the error in the exit wave.
...
#         #exit_waves = uk[:, -self.block_size:]
#         exit_wave_error = np.zeros_like(self.convert_to_tensor_form(uk), dtype=complex)
#         exit_waves = self.convert_to_tensor_form(uk)[...,-1].reshape(self.total_projection_scans, 
#                                                                      self.block_size)

#         if self.phase_retrieval:
#             emodel = self._apply_phase_retrieval_constraint(exit_waves,projection_idx=projection_idx)
#             exit_wave_error[..., :, -1] = (exit_waves - emodel).reshape(self.num_angles,
#                                                                            self.num_probes,
#                                                                              self.block_size)
...
#         else:  # Known Phase
#             exit_wave_error[..., :, -1] = (exit_waves - self.data[projection_idx,...]).reshape(self.num_angles,
#                                                                            self.num_probes,
#                                                                              self.block_size)
...
#         Parameters:
#         u (ndarray): Input array to be converted. shape: (num_angles, num_probes, nx, nz)

#         Returns:
#         ndarray: Block-formatted array. (total_projection_scans, nx*nz)
#         """
#         # 2. Remove initial condition
#         u = u[:, :, :, 1:]  # shape: (num_angles, num_probes, block_size, nz - 1)

#         # 3. Transpose axes
#         u = u.transpose(0, 1, 3, 4) # shape: (num_angles, num_probes, nz - 1, block_size)

#         # 4. Flatten last two dims
#         # shape: (total_scans, block_size * (nz - 1))
#         u = u.reshape(self.total_projection_scans, -1)

#         return u


#     def _apply_phase_retrieval_constraint(self, exit_waves: np.ndarray,projection_idx: int=0) -> np.ndarray:
#         """
#         Apply a Gerchberg–Saxton-like phase retrieval constraint in Fourier space.
...
#         # Select measured amplitude data
#         target_amplitude = np.sqrt(self.data[projection_idx,...])
