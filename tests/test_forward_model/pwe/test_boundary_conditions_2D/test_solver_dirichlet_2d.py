...
def compute_error(nx, nz,Solver):
    """Computes the L-Infinity norm of the error between the exact and computed solutions in 2D."""
    k = 100
    ...
    sim_config = SimulationConfig(
        probe_type="dirichlet_test",      # triggers sinusoidal probe generation
    ...
    solver = Solver(simulation_space, obj, probes, bc_type="dirichlet")
    ...
    exact_solution = u_nm(1, k)(X, Z) + 0.5 * u_nm(5, k)(X, Z) + 0.2 * u_nm(9, k)(X, Z)
...
def test_error(Solver, request):
    ...
    # rmse_errors = []
    inf_norms = []
    bc_type = "Dirichlet 2D"  # Boundary condition type

    print(f"\n=== CONVERGENCE STUDY: {bc_type.upper()} BOUNDARY CONDITIONS ===")
    print(f"Solver: {Solver}\n")
    for i, nx in enumerate(nx_values):
        nz = nx  # Crank Nicolson method requires dz ~ dx^2 for second order accuracy
        nz_values.append(nz)
