...
def plot_convergence(nx_values, inf_norms, bc_type, Solver):
    ...
    plt.loglog(
        nx_values, inf_norms, "bo-", linewidth=2, markersize=8, label=r"$L^\infty$ norm"
    )
    ...
    plt.loglog(
        nx_values, theoretical_line, "r--", alpha=0.7, label="Theoretical slope = 2"
    )
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def test_error_convergence(Solver, bc_type, request, limits_3d):
    ...
    if bc_type == BoundaryType.DIRICHLET:
        nx_values.append(
            64
        )  # Finer mesh for Dirichlet BC to achieve theortical convergence
    nz_values = nx_values
    inf_norms, observed_rates = error_rates(
        nx_values, nz_values, Solver, bc_type, request, limits_3d
    )
    ...
    e_message = f"Expected ~2nd order convergence, got {observed_rates[-1]:.2f}"
    assert 1.5 <= observed_rates[-1] <= 2.5, e_message

    # Verify the other slower solvers match the above solution on coarse mesh.
    nx = 8
    nz = 8
    numerical_iterative = compute_numerical(
        nx, nz, PWEIterativeLUSolver, bc_type, limits_3d
    )
    numerical_FullPinT = compute_numerical(
        nx, nz, PWEFullPinTSolver, bc_type, limits_3d
    )
    ...
    assert np.allclose(numerical_iterative, numerical_FullLU, atol=1e-6), e_message
    # Reduce tolerance for FullPinT due to iterative solver inaccuracies

    e_message = "Convergent FullLUSolver and FullPinTSolver solutions do not match for Dirichlet BC"
    assert np.allclose(numerical_iterative, numerical_FullPinT, atol=1e-4), e_message
