...
def test_opticalshape_creates_1d_instance():
    ...
    assert isinstance(shape, OpticalShape1D), "Should create OpticalShape1D for 1D centre input."
...
def test_opticalshape_creates_2d_instance():
    ...
    assert isinstance(shape, OpticalShape2D), "Should create OpticalShape2D for 2D centre input."
...
def test_object_out_of_bounds_raises_assertion():
    sim = SimulationSpace1D(nx=100, nz=100)
...
def test_object_out_of_bounds_raises_assertion_2d():
    sim = SimulationSpace2D(nx=50, ny=50, nz=50)
...
def test_field_shape_matches_simulation_space_1d():
    sim = SimulationSpace1D(nx=64, nz=64)
    shape = OpticalShape(
        centre_scale=[0.5, 0.5],
        shape="circle",
        refractive_index=1.5,
...
def test_field_shape_matches_simulation_space_2d():
    sim = SimulationSpace2D(nx=32, ny=32, nz=32)
    shape = OpticalShape(
        centre_scale=[0.5, 0.5, 0.5],
        shape="cuboid",
        refractive_index=1.5,
